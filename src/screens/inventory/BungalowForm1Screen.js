...
// Name validation: Only letters, spaces, and hyphens (at least 2 characters)
const validateName = (name) => {
  const nameRegex = /^[A-Za-z\s\-]{2,}$/;
  return nameRegex.test(name.trim());
};

// Phone validation: Indian phone number (10 digits, starting with 6-9)
const validatePhone = (phone) => {
  const phoneRegex = /^[6-9]\d{9}$/;
  return phoneRegex.test(phone.trim());
};

// Address validation: At least 5 characters with letters, numbers, spaces, commas
...
const VillaForm1Screen = ({ navigation }) => {
  ...
  const [owners, setOwners] = useState([{ id: 1, name: '', phone: '', nameError: '', phoneError: '' }]);
  const [representatives, setRepresentatives] = useState([
    { id: 1, name: '', phone: '', nameError: '', phoneError: '' },
  ]);

  const updateEntry = (list, setList, id, field, text) => {
    setList(list.map((e) => {
      if (e.id === id) {
        const updated = { ...e, [field]: text };
        // Clear error when user starts typing
        if (field === 'name') updated.nameError = '';
        if (field === 'phone') updated.phoneError = '';
        return updated;
      }
      return e;
    }));
  ...
  const validateOwnerEntry = (owner) => {
    let isValid = true;
    const updated = { ...owner };
    
    if (!validateName(owner.name)) {
      updated.nameError = 'Name should contain only letters and spaces (min 2 chars)';
      isValid = false;
    } else {
      updated.nameError = '';
    }
    ...
    if (!validatePhone(owner.phone)) {
      updated.phoneError = 'Enter valid 10-digit phone number starting with 6-9';
      isValid = false;
    } else {
      updated.phoneError = '';
    }
  ...
  const validateRepresentativeEntry = (rep) => {
    let isValid = true;
    const updated = { ...rep };
    
    if (rep.name && !validateName(rep.name)) {
      updated.nameError = 'Name should contain only letters and spaces (min 2 chars)';
      isValid = false;
    } else {
      updated.nameError = '';
  ...
  const addOwner = () =>
    setOwners((prev) => [...prev, { id: Date.now(), name: '', phone: '', nameError: '', phoneError: '' }]);

  const addRepresentative = () =>
    setRepresentatives((prev) => [...prev, { id: Date.now(), name: '', phone: '', nameError: '', phoneError: '' }]);
  ...
  const validateAllFields = () => {
    ...
    if (!validateAddress(address)) {
      setAddressError('Please enter a valid address (min 5 characters)');
      isValid = false;
    } else {
      setAddressError('');
    }
    ...
    const validatedOwners = owners.map(owner => {
      const result = validateOwnerEntry(owner);
      if (!result.isValid) isValid = false;
      return result;
    });
    setOwners(validatedOwners);

    // Validate all representatives (only if they have data)
    const validatedReps = representatives.map(rep => {
      // Skip validation if both fields are empty (optional entry)
      if (!rep.name.trim() && !rep.phone.trim()) {
        return { ...rep, nameError: '', phoneError: '', isValid: true };
      }
      const result = validateRepresentativeEntry(rep);
      if (!result.isValid) isValid = false;
      return result;
    });
    setRepresentatives(validatedReps);
  ...
  const handleContinue = () => {
    if (validateAllFields()) {
      ...
      navigation.navigate('BungalowForm2Screen');
    } else {
      Alert.alert(
        'Validation Error',
    ...
    <SafeAreaView style={styles.container}>
      ...
      <KeyboardAvoidingView
        ...
        <ScrollView
          ...
          {/* Continue button — scrolls with the rest of the form */}
          <TouchableOpacity
...
export default VillaForm1Screen;
