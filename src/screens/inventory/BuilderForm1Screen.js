...
// Regex patterns
// ---------------------------------------------------------------------------
const NAME_REGEX = /^[A-Za-z][A-Za-z\s.'-]{1,49}$/; // letters, 2-50 chars
const PHONE_REGEX = /^[6-9]\d{9}$/; // 10-digit Indian mobile number
const ADDRESS_REGEX = /^.{5,200}$/s; // 5-200 chars, any content
...
const LabeledDropdown = ({ label, value, options, onSelect, error }) => {
...
const validateName = (value) => {
  ...
  if (!NAME_REGEX.test(value.trim())) {
    return 'Enter a valid name (letters only, 2-50 characters)';
  }
  return '';
};
...
const BuilderForm1Screen = ({ navigation }) => {
  ...
  // Errors keyed by entry id: { [id]: { name: '', phone: '' } }
  ...
  const clearFieldError = (errors, setErrors, id, field) => {
  ...
  const validateEntryField = (errors, setErrors, id, field, value) => {
    const message = field === 'name' ? validateName(value) : validatePhone(value);
    setErrors((prev) => ({
      ...prev,
      [id]: { ...prev[id], [field]: message },
    }));
    return message === '';
  };
  ...
  const validateEntryList = (list, setErrors) => {
    let allValid = true;
    const newErrors = {};
    list.forEach((entry) => {
      const nameMsg = validateName(entry.name);
      const phoneMsg = validatePhone(entry.phone);
      newErrors[entry.id] = { name: nameMsg, phone: phoneMsg };
      if (nameMsg || phoneMsg) allValid = false;
    });
    setErrors(newErrors);
    return allValid;
  };
  ...
  const handleContinue = () => {
    const ownersValid = validateEntryList(owners, setOwnerErrors);
    const repsValid = validateEntryList(representatives, setRepErrors);
    const addressMsg = validateAddress(address);
    setAddressError(addressMsg);

    if (!ownersValid || !repsValid || addressMsg) {
      return;
    }
    ...
    navigation.navigate('BuilderForm2');
  };
      ...
      <KeyboardAvoidingView
        ...
        <ScrollView
          ...
          {owners.map((owner, index) => (
            <React.Fragment key={owner.id}>
              <LabeledInput
                ...
                value={owner.name}
                onChangeText={(t) => {
                  updateEntry(owners, setOwners, owner.id, 'name', t);
                  clearFieldError(ownerErrors, setOwnerErrors, owner.id, 'name');
                }}
                onBlur={() =>
                  validateEntryField(ownerErrors, setOwnerErrors, owner.id, 'name', owner.name)
                }
                placeholder="Anand"
                error={ownerErrors[owner.id] && ownerErrors[owner.id].name}
              />
              <LabeledInput
                label="Phone number"
                value={owner.phone}
                onChangeText={(t) => {
                  updateEntry(owners, setOwners, owner.id, 'phone', t);
                  clearFieldError(ownerErrors, setOwnerErrors, owner.id, 'phone');
                }}
                onBlur={() =>
                  validateEntryField(ownerErrors, setOwnerErrors, owner.id, 'phone', owner.phone)
                }
                placeholder="92892378292"
                keyboardType="phone-pad"
                error={ownerErrors[owner.id] && ownerErrors[owner.id].phone}
              />
            </React.Fragment>
          ))}
            ...
            <React.Fragment key={rep.id}>
              <LabeledInput
                ...
                onChangeText={(t) => {
                  ...
                  clearFieldError(repErrors, setRepErrors, rep.id, 'name');
                }}
                onBlur={() =>
                  validateEntryField(repErrors, setRepErrors, rep.id, 'name', rep.name)
                }
                placeholder="Prashant"
                error={repErrors[rep.id] && repErrors[rep.id].name}
              />
              <LabeledInput
                label="Phone number"
                value={rep.phone}
                onChangeText={(t) => {
                  ...
                  clearFieldError(repErrors, setRepErrors, rep.id, 'phone');
                }}
                onBlur={() =>
                  validateEntryField(repErrors, setRepErrors, rep.id, 'phone', rep.phone)
                }
                placeholder="92892378292"
                keyboardType="phone-pad"
                error={repErrors[rep.id] && repErrors[rep.id].phone}
              />
            </React.Fragment>
          ))}
          ...
          <LabeledInput
            ...
            onChangeText={(t) => {
              setAddress(t);
              if (addressError) setAddressError(validateAddress(t));
            }}
