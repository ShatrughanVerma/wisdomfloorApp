...
const OfficeFormScreen = ({ navigation }) => {
  ...
  const [workstations, setWorkstations] = useState('12');
  const [cabins, setCabins] = useState('12');

  const [numberOfAC, setNumberOfAC] = useState('12');
  const [serverRoom, setServerRoom] = useState('Yes');

  const [toilets, setToilets] = useState('');
  const [pantry, setPantry] = useState('Yes');

  const [superArea, setSuperArea] = useState('123');
  const [coveredArea, setCoveredArea] = useState('123');
  ...
  const handleSubmit = () => {
    ...
    console.log('Office form payload:', payload);
  };
      ...
      <KeyboardAvoidingView
        ...
        <ScrollView
          ...
          <Row>
            <Col style={{ marginRight: scale(12) }}>
              <LabeledInput
                label="No. of workstation"
                value={workstations}
                onChangeText={setWorkstations}
