...
// Regex patterns
// ---------------------------------------------------------------------------
const NAME_REGEX = /^[A-Za-z\s.]{2,50}$/; // e.g. owner / representative names
const PHONE_REGEX = /^\d{10}$/; // 10 digit phone number
...
const validateName = (value, label) => {
  ...
  if (!NAME_REGEX.test(value.trim())) return `Enter a valid ${label.toLowerCase()}`;
  return '';
};

const validatePhone = (value, label = 'Phone number') => {
  if (!value || !value.trim()) return `${label} is required`;
  if (!PHONE_REGEX.test(value.trim())) return `Enter a valid 10-digit ${label.toLowerCase()}`;
  return '';
};
...
const VillaForm1Screen = ({ navigation }) => {
  ...
  // ---- Error state, keyed by field / entry id ----
  ...
  const handleContinue = () => {
    ...
    owners.forEach((owner, index) => {
      newErrors[`ownerName_${owner.id}`] = validateName(
        owner.name,
        index === 0 ? 'Name of owner' : `Name of owner ${index + 1}`
      );
      newErrors[`ownerPhone_${owner.id}`] = validatePhone(owner.phone);
    });

    representatives.forEach((rep, index) => {
      newErrors[`repName_${rep.id}`] = validateName(
        rep.name,
        index === 0 ? 'Name of the representative' : `Name of the representative ${index + 1}`
      );
      newErrors[`repPhone_${rep.id}`] = validatePhone(rep.phone);
    });
    ...
    navigation.navigate('VillaForm2Screen');
  };
            ...
            <React.Fragment key={owner.id}>
              <LabeledInput
                ...
                onChangeText={(t) => {
                  ...
                  clearFieldErrorIfSet(`ownerName_${owner.id}`);
                }}
                onBlur={() =>
                  setFieldError(
                    `ownerName_${owner.id}`,
                ...
                placeholder="Anand"
                error={errors[`ownerName_${owner.id}`]}
              />
              <LabeledInput
                label="Phone number"
                value={owner.phone}
                onChangeText={(t) => {
                  ...
                  clearFieldErrorIfSet(`ownerPhone_${owner.id}`);
                }}
                onBlur={() =>
                  setFieldError(`ownerPhone_${owner.id}`, validatePhone(owner.phone))
                }
                placeholder="92892378292"
                keyboardType="phone-pad"
                error={errors[`ownerPhone_${owner.id}`]}
              />
            </React.Fragment>
          ))}
            ...
            <React.Fragment key={rep.id}>
              <LabeledInput
                ...
                onChangeText={(t) => {
                  ...
                  clearFieldErrorIfSet(`repName_${rep.id}`);
                }}
                onBlur={() =>
                  setFieldError(
                    `repName_${rep.id}`,
              ...
              <LabeledInput
                ...
                onChangeText={(t) => {
                  ...
                  clearFieldErrorIfSet(`repPhone_${rep.id}`);
                }}
                onBlur={() =>
                  setFieldError(`repPhone_${rep.id}`, validatePhone(rep.phone))
                }
                placeholder="92892378292"
                keyboardType="phone-pad"
                error={errors[`repPhone_${rep.id}`]}
              />
            </React.Fragment>
          ))}
